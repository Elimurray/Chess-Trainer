...
3. Add the ECO code to the opening select page groupings

Verify moves are legal by running them through `chess.js` in a test:
```ts
// In a Vitest test
const chess = new Chess();
for (const move of myOpening.moves) {
  const result = chess.move(move);
  expect(result).not.toBeNull(); // null = illegal move
}
```
