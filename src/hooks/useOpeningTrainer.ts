...
import type { OpeningLine, TrainingState, PlayedMove, DeviationInfo } from '../types/opening';
...
type Action =
  ...
  | { type: 'ENGINE_PLAYED'; move: PlayedMove; nextFen: string; nextIndex: number }
  | { type: 'USER_CORRECT'; move: PlayedMove; nextFen: string; nextIndex: number }
  | { type: 'USER_DEVIATION'; move: PlayedMove; deviation: DeviationInfo; fen: string }
...
export function useOpeningTrainer(opening: OpeningLine): UseOpeningTrainerResult {
  ...
  // Auto-play engine book moves whenever moveIndex advances.
  useEffect(() => {
    ...
    if (!result) {
      console.error(`[useOpeningTrainer] Illegal engine book move at index ${moveIndex}: ${san}`);
      return;
    }

    dispatch({
      type: 'ENGINE_PLAYED',
      move: { san: result.san, uci: result.from + result.to + (result.promotion ?? ''), fen: chess.fen() },
      nextFen: chess.fen(),
  ...
  const makeMove = useCallback(
    (from: string, to: string, promotion?: string): boolean => {
      ...
      if (normSan(result.san) === normSan(expectedSan)) {
        ...
        dispatch({ type: 'USER_DEVIATION', move: playedMove, deviation, fen: chess.fen() });
